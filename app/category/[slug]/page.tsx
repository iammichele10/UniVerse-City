...
function slugToCategory(slug: string, categories: string[]) {
  return categories.find(
    (c) =>
      c.toLowerCase().replace(/\s+/g, '-').replace(/&/g, 'and') === slug,
  );
}
  ...
  return (
    <main className="mx-auto max-w-3xl">
      ...
      <div className="px-4 py-8 sm:px-6 sm:py-10">
        <h1 className="mb-1 font-serif text-xl text-ink sm:text-2xl">
          {category}
        </h1>
        ...
        <div className="space-y-5">
          {posts.map((post) => (
            ...
            >
              <div className="min-w-0 flex-1">
                <h2 className="mb-1 font-serif text-base text-ink sm:text-lg">
                  {post.title}
                </h2>
          ...
          {posts.length === 0 && (
            <p className="text-muted">No posts in this section yet.</p>
          )}
        </div>
      </div>
    </main>
