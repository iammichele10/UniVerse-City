...
export default function AdminSettingsPage() {
  ...
  function removeMember(id: string) {
    setSettings({
      ...settings,
      team: settings.team.filter((member) => member.id !== id),
    });
  }
  ...
  return (
    <div className="max-w-2xl space-y-8">
      ...
      <div>
        <Link
          href="/admin"
          className="mb-4 inline-flex items-center gap-2 rounded-md border border-rule bg-white px-3 py-2 text-sm text-navy transition hover:bg-paper"
        >
          ← Back to Admin
        </Link>
        ...
        <p className="mt-1 text-xs text-muted">
          Manage the site identity, About page, categories, and team.
        </p>
      </div>
      ...
      <section className="space-y-3">
        <div>
          ...
          <p className="mt-1 text-xs text-muted">
            This introduction appears at the top of the public About page.
          </p>
        </div>
      ...
      <section>
        <div className="mb-3 flex items-end justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-muted">
              Only name, role, and photo are shown publicly. Use the arrows to
              reorder.
            </p>
          </div>
                  ...
                  <div className="flex flex-wrap items-center gap-2 text-xs">
                    ...
                    {member.photoUrl && (
                      <button
                        ...
                        className="px-2 py-2 text-red-700 underline"
                      >
                        Remove photo
                      </button>
                    )}
              ...
              <div className="mt-3 flex flex-wrap items-center justify-between gap-2 border-t border-rule pt-3">
                <div className="flex gap-1">
                  <button
                    ...
                    className="rounded border border-rule px-2 py-1 text-sm disabled:opacity-30"
                    aria-label="Move up"
                  >
                    ↑
                  </button>
      ...
      <section>
        ...
        <ul className="mb-3 space-y-2">
          {settings.categories.map((cat) => (
            <li
              key={cat}
              className="flex items-center justify-between rounded-md border border-rule bg-white px-3 py-2 text-sm"
            >
              {cat}
